...
validate_features_schema() {
    ...
    if jq -e '.features' "$config_file" >/dev/null 2>&1; then
        # Validate feature ID format - collect IDs first to avoid subshell issues
        local invalid_feature_ids=()
        while IFS= read -r feature_id; do
            if [[ -n "$feature_id" ]] && [[ ! "$feature_id" =~ ^[a-zA-Z0-9._-]+/[a-zA-Z0-9._-]+/[a-zA-Z0-9._-]+$ ]] && [[ ! "$feature_id" =~ ^ghcr\.io/ ]]; then
                invalid_feature_ids+=("$feature_id")
            fi
        done < <(jq -r '.features | keys[]' "$config_file" 2>/dev/null)

        # Now add the warnings in the parent shell
        for feature_id in "${invalid_feature_ids[@]}"; do
        ...
        local feature_with_objects=()
        while IFS= read -r feature_id; do
            if [[ -n "$feature_id" ]]; then
                feature_with_objects+=("$feature_id")
            fi
        done < <(jq -r '.features | to_entries[] | select(.value | type == "object") | .key' "$config_file" 2>/dev/null)

        for feature_id in "${feature_with_objects[@]}"; do
