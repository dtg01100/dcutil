...
PY
    ...
    if command -v curl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
        info "Fetching official devcontainer templates from GitHub..." >&2
...
PY
    ...
    # Fetch from GitHub API like VSCode does
    if command -v curl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
        info "Fetching official devcontainer features from GitHub..." >&2
        local features_json="[]"
...
sanitize_features_json() {
    ...
    # Strip comments from JSON before processing with jq
    # Use a safer regex that only removes // comments at the start or after whitespace
    local clean_json
    clean_json=$(sed -E 's|^[[:space:]]*//.*||g; s|[[:space:]]+//.*||g' "$dev_file")
    
    # Only proceed if we have valid JSON (ignoring comments we just stripped)
    if ! echo "$clean_json" | jq . >/dev/null 2>&1; then
        # Fallback: if cleaning failed, try the original file (maybe it has no comments)
        if ! jq . "$dev_file" >/dev/null 2>&1; then
            warning "Could not parse $dev_file, skipping feature sanitization"
            return 0
        fi
        clean_json=$(cat "$dev_file")
    fi
    
    echo "$clean_json" | jq --arg pref "$features_json" '
        ($pref | fromjson) as $prefs |
        def mapkey(k):
            if k | test("^[0-9]+$") then
                (k | tonumber - 1) as $i | ($prefs[$i].id // k)
            else k end;
        .features = (.features // {} | to_entries | map(.key = mapkey(.key)) | from_entries)
    ' > "$temp_file" && mv "$temp_file" "$dev_file"
}
