...
validate_command() {
    local cmd="${1:-}"
    local valid_commands="up down restart enter build rebuild clean status stats logs list run init check ssh volumes compose features advanced integration merging userprobe hostrequirements shutdown schema podman version help completion test verify-dialog edit menu"
