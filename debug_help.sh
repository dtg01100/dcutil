...
if [ -f "$SCRIPT_DIR/lib/environment.sh" ]; then
    source "$SCRIPT_DIR/lib/environment.sh"
fi
# userprobe module is optional
if [ -f "$SCRIPT_DIR/lib/userprobe.sh" ]; then
    source "$SCRIPT_DIR/lib/userprobe.sh"
fi
# hostrequirements module is optional
if [ -f "$SCRIPT_DIR/lib/hostrequirements.sh" ]; then
    source "$SCRIPT_DIR/lib/hostrequirements.sh"
fi
...
test_dcutil_improvements() {
    ...
    if grep -q "mktemp" "$SCRIPT_DIR/lib/volumes.sh"; then
    ...
    if grep -q "sleep 1" "$SCRIPT_DIR/lib/security.sh"; then
...
complete -F _source_dcutil_completion dcutil
BASH_EOF
        elif [ "$shell" = "zsh" ]; then
            cat << 'ZSH_EOF'
#compdef dcutil
...
if [ "$COMMAND" = "run" ]; then
    ...
    # Volumes command doesn't need project directory
    determine_project_dir
    shift 1
...
EOF
        ...
        "")
            ...
            if command -v show_advanced_features_info >/dev/null 2>&1; then
                ...
                error_exit "No advanced features configured or advanced features module not available." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        *)
            error_exit "Unknown advanced subcommand: ${1:-}. Use 'dcutil advanced help' for usage." "$EXIT_INVALID_ARGS"
            ;;
    esac
...
EOF
        ...
        "")
            ...
            if command -v show_tool_integration_info >/dev/null 2>&1; then
                ...
                error_exit "No integration features configured or integration module not available." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        *)
            error_exit "Unknown integration subcommand: ${1:-}. Use 'dcutil integration help' for usage." "$EXIT_INVALID_ARGS"
            ;;
    esac
...
EOF
        ...
        "")
            ...
            ;;
        *)
            error_exit "Unknown merging subcommand: ${1:-}. Use 'dcutil merging help' for usage." "$EXIT_INVALID_ARGS"
            ;;
    esac
...
EOF
        ...
        "")
            ...
            ;;
        *)
            error_exit "Unknown userprobe subcommand: ${1:-}. Use 'dcutil userprobe help' for usage." "$EXIT_INVALID_ARGS"
            ;;
    esac
elif [ "$COMMAND" = "features" ]; then
    # Features command doesn't need project directory
    determine_project_dir
    shift 1
    ...
    # Check for features subcommands
    case "${1:-}" in
        "install")
            # Install features
            if command -v install_features >/dev/null 2>&1; then
                ...
                error_exit "Features installation not available. No features configuration found." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        "info")
            ...
            if command -v show_features_info >/dev/null 2>&1; then
                ...
                error_exit "Features info not available. No features configuration found." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        "validate")
            ...
            if command -v validate_features_config >/dev/null 2>&1; then
                ...
                error_exit "Features validation not available. No features configuration found." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        "clean")
            ...
            if command -v clean_features_cache >/dev/null 2>&1; then
                ...
                error_exit "Features clean not available. No features configuration found." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        "update")
            ...
            if command -v update_features >/dev/null 2>&1; then
                ...
                error_exit "Features update not available. No features configuration found." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        "check-updates")
            ...
            if command -v check_features_updates >/dev/null 2>&1; then
                ...
                error_exit "Features update check not available. No features configuration found." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        "help"|"-h"|"--help")
...
EOF
        ...
        "")
            ...
            if command -v install_features >/dev/null 2>&1; then
                ...
                error_exit "No features configured or features module not available." "$EXIT_CONFIG_ERROR"
            fi
            exit $EXIT_SUCCESS
            ;;
        *)
    ...
    if [ -f ".devcontainer/devcontainer.json" ] || [ -f ".devcontainer.json" ]; then
        error_exit "Devcontainer configuration already exists in current directory. Use 'dcutil clean' first to remove existing configuration" "$EXIT_CONFIG_ERROR"
    fi
    PROJECT_DIR="$(pwd)"
    shift 1
    init_mode "$@"
    ...
    "volumes")
        ...
        case "$subcommand" in
            ...
            "help"|"-h"|"--help")
                ...
                ;;
            *)
                error_exit "Unknown volumes subcommand: $subcommand. Use 'dcutil volumes help' for usage." "$EXIT_INVALID_ARGS"
                ;;
        esac
        ;;
    "compose")
        ...
        case "$subcommand" in
            ...
            "help"|"-h"|"--help")
                ...
                ;;
            *)
                error_exit "Unknown compose subcommand: $subcommand. Use 'dcutil compose help' for usage." "$EXIT_INVALID_ARGS"
                ;;
        esac
        ;;
    "advanced")
        ...
        case "$subcommand" in
            ...
            "help"|"-h"|"--help")
                ...
                ;;
            *)
                error_exit "Unknown advanced subcommand: $subcommand. Use 'dcutil advanced help' for usage." "$EXIT_INVALID_ARGS"
                ;;
        esac
        ;;
    "integration")
        ...
        case "$subcommand" in
            ...
            "help"|"-h"|"--help")
                ...
                ;;
            *)
                error_exit "Unknown integration subcommand: $subcommand. Use 'dcutil integration help' for usage." "$EXIT_INVALID_ARGS"
                ;;
        esac
        ;;
    "merging")
        ...
        case "$subcommand" in
            ...
            "help"|"-h"|"--help")
                ...
                ;;
            *)
                error_exit "Unknown merging subcommand: $subcommand. Use 'dcutil merging help' for usage." "$EXIT_INVALID_ARGS"
                ;;
        esac
        ;;
    "userprobe")
        ...
        case "$subcommand" in
            ...
            "help"|"-h"|"--help")
                ...
                ;;
            *)
                error_exit "Unknown userprobe subcommand: $subcommand. Use 'dcutil userprobe help' for usage." "$EXIT_INVALID_ARGS"
                ;;
        esac
        ;;
    "hostrequirements")
        ...
        case "$subcommand" in
            ...
            "help"|"-h"|"--help")
                ...
                ;;
            *)
                error_exit "Unknown hostrequirements subcommand: $subcommand. Use 'dcutil hostrequirements help' for usage." "$EXIT_INVALID_ARGS"
                ;;
        esac
        ;;
    "shutdown")
...
Devcontainer management tool for enhanced development environments.

Quick Start:
  dcutil up                    Start devcontainer
  dcutil enter                 Enter running devcontainer
  dcutil down                  Stop devcontainer
  dcutil restart               Restart devcontainer

Commands:
  up          Start the devcontainer (build if needed)
  down        Stop and clean up the devcontainer
  restart     Restart the devcontainer
  enter       Enter the running devcontainer
  build       Build the devcontainer image
  clean       Clean up devcontainer resources
  status      Show devcontainer status
  logs        Show devcontainer logs
  list        List available devcontainers
  run         Run a command in the devcontainer
  init        Initialize devcontainer setup
  install-agent Install authentication agent in container
  ssh         SSH key management
  volumes     Volume management commands
  compose     Docker Compose environment commands
  features    Devcontainer Features management
  advanced    Advanced devcontainer features
  integration Tool integration features
  merging     Image metadata merging
  userprobe   User environment probing
  ...
  hostrequirements Host system requirements validation
  shutdown    Container shutdown actions
  rebuild     Rebuild devcontainer with options
...
Examples:
  ...
  dcutil volumes list          # List volumes
  dcutil features install      # Install features
...
Documentation: https://github.com/microsoft/devcontainer-spec
EOF
}
